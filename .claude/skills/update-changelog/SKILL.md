...
You are writing changelog entries for the **Readium Kotlin Toolkit**.
...
- Mention the **public API surface** (type name, method name) in backticks.
...
- Link to an existing guide when the change is non-trivial to adopt:
  - Migration steps: `[the migration guide](docs/migration-guide.md)` (or a specific anchor)
