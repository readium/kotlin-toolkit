...
| Notification when jumping to a distant location | ✅ | ❌ | The legacy `Listener.onJumpToLocator` lets apps record the previous location to offer a "go back" button; no equivalent in the new API |
...
| Preferences editor for building a user settings UI | ✅ | ❌ | No `PreferencesEditor` for the new preference sets |
...
| Custom `RsProperties` overrides | ✅ | ❌ | Exposed via `Configuration.readiumCssRsProperties` in legacy; `internal` in the new navigator |
...
| Activable decorations (tap events) | ✅ | ⚠️ | `DecorationListener.onDecorationActivated` exists, but `defaultDecorationListener()` is a no-op placeholder |
...
| Tapped element info (e.g. image preview) | ✅ | ❌ | Legacy `TapEvent.targetElement`; new `TapEvent` only carries an offset |
...
| `firstVisibleElementLocator()` | ✅ | ❌ | Needed for TTS synchronization |
