...
class HttpFetcher : Fetcher {
    ...
    override fun fetch(link: Link): ResourceHandle? =
        if (link.href.startsWith("http")) HttpResourceHandle(link.href) else null
}

private class HttpResourceHandle(href: String): ResourceHandle(href) {

    private val headConnection: HttpURLConnection? by lazy {
        (URL(href).openConnection() as? HttpURLConnection)?.apply { requestMethod = "HEAD" }
    }

    private val getConnection: HttpURLConnection? by lazy {
        (URL(href).openConnection() as? HttpURLConnection)?.apply { requestMethod = "GET" }
    }
