...
import org.readium.r2.shared.util.archive.channel.compress.archivers.EntryStreamOffsets;
import org.readium.r2.shared.util.archive.channel.jvm.SeekableByteChannel;
import org.readium.r2.shared.util.archive.channel.compress.utils.BoundedArchiveInputStream;
import org.readium.r2.shared.util.archive.channel.compress.utils.BoundedSeekableByteChannelInputStream;
import org.readium.r2.shared.util.archive.channel.compress.utils.CountingInputStream;
import org.readium.r2.shared.util.archive.channel.compress.utils.IOUtils;
import org.readium.r2.shared.util.archive.channel.compress.utils.InputStreamStatistics;
 ...
 * {@code org.readium.r2.shared.util.archive.remote.zip.ZipArchiveEntry}
...
public class ZipFile implements Closeable {
    ...
    private static final Comparator<ZipArchiveEntry> offsetComparator =
            (e1, e2) -> {
                final int diskNumberStartComparison =
                        Long.compare(e1.getDiskNumberStart(), e2.getDiskNumberStart());
                if (diskNumberStartComparison != 0) {
                    return diskNumberStartComparison;
                } else {
                    return Long.compare(e1.getLocalHeaderOffset(), e2.getLocalHeaderOffset());
                }
            };
    ...
    private void fillNameMap() {
        for (ZipArchiveEntry ze: entries) {
            ...
            final LinkedList<ZipArchiveEntry> entriesOfThatName =
                    Objects.requireNonNull(nameMap.get(name));
