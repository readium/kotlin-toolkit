 ...
 *
 */
package org.readium.r2.shared.util.archive.channel.compress.archivers.zip;

import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipConstants.DATA_DESCRIPTOR_MIN_VERSION;
import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipConstants.DEFLATE_MIN_VERSION;
import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipConstants.DWORD;
import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipConstants.INITIAL_VERSION;
import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipConstants.SHORT;
import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipConstants.WORD;
import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipConstants.ZIP64_MAGIC;
import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipConstants.ZIP64_MAGIC_SHORT;
import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipConstants.ZIP64_MIN_VERSION;
import static org.readium.r2.shared.util.archive.channel.compress.archivers.zip.ZipLong.putLong;

import org.readium.r2.shared.util.archive.channel.compress.archivers.ArchiveEntry;
import org.readium.r2.shared.util.archive.channel.compress.archivers.ArchiveOutputStream;
import org.readium.r2.shared.util.archive.channel.jvm.SeekableByteChannel;
import org.readium.r2.shared.util.archive.channel.compress.utils.ByteUtils;
...
public class ZipArchiveOutputStream extends ArchiveOutputStream {
    ...
    public ZipArchiveOutputStream(final File file, final long zipSplitSize) throws IOException {
        def = new Deflater(level, true);
        this.outputStream = new ZipSplitOutputStream(file, zipSplitSize);
    ...
    private byte[] createCentralFileHeader(final ZipArchiveEntry ze, final ByteBuffer name,
        ...
        if(isSplitZip) {
            if (ze.getDiskNumberStart() >= ZIP64_MAGIC_SHORT || zip64Mode == Zip64Mode.Always) {
                ZipShort.putShort(ZIP64_MAGIC_SHORT, buf, CFH_DISK_NUMBER_OFFSET);
            } else {
     ...
     * when calling {@link #putArchiveEntry} -
