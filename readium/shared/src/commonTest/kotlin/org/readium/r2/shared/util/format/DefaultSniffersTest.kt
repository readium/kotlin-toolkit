...
class DefaultSniffersTest {
    ...
    @Test
    fun `EpubDrmSniffer doesn't recognize EPUB with empty encryption xml`() = runTest {
        assertEquals(
            epubFormat,
  ...
  <EncryptedData xmlns="http://www.w3.org/2001/04/xmlenc#">
    ...
    @Test
    fun `Sniff Adobe ADEPT from rights xml`() = runTest {
        assertEquals(
            epubFormat.copy(specification = epubFormat.specification + Specification.Adept),
            EpubDrmSniffer.sniffContainer(
                format = epubFormat,
    ...
    @Test
    fun `Sniff LCP protected EPUB`() = runTest {
        assertEquals(
    ...
    @Test
    fun `Sniff LCP protected EPUB missing the license`() = runTest {
        assertEquals(
            epubFormat.copy(specification = epubFormat.specification + Specification.Lcp),
            EpubDrmSniffer.sniffContainer(
                format = epubFormat,
