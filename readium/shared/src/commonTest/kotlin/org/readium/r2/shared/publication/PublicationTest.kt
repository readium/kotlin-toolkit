...
class PublicationTest {
    ...
    @Test fun `get the default empty positions`() = runTest {
        assertEquals(emptyList<Locator>(), createPublication().positions())
    }

    @Test fun `get the positions computed from the PositionsService`() = runTest {
        assertEquals(
    ...
    @Test fun `get the positionsByReadingOrder computed from the PositionsService`() = runTest {
        assertEquals(
            listOf(
                listOf(
            ...
            ).let { it.positionsByReadingOrder() }
        )
    }
