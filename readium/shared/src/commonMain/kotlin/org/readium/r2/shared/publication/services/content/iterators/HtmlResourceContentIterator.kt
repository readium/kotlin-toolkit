...
public class HtmlResourceContentIterator internal constructor(
    ...
    private class ContentParser(
        ...
        override fun tail(node: Node, depth: Int) {
            if (node is TextNode && node.getWholeText().isNotBlank()) {
                ...
                val text = Parser.unescapeEntities(node.getWholeText(), false)
                rawTextAcc += text
...
private fun StringBuilder.appendNormalisedWhitespace(
    ...
    for (c in string) {
        if (isWhitespace(c.code)) {
            ...
            // Appending char by char is safe with surrogate pairs, which are never whitespace nor
