...
import org.readium.r2.shared.util.data.decodeImage
...
public interface CoverService : Publication.Service {
    ...
    /**
     * Returns the publication cover as a [ReadiumImage] at its maximum size.
     *
     * If the cover is not a bitmap format (e.g. SVG), it should be scaled down to fit the screen.
     */
    public suspend fun cover(): ReadiumImage?

    /**
     *  Returns the publication cover as a [ReadiumImage], scaled down to fit the given [maxSize].
     */
    public suspend fun coverFitting(maxSize: ImageSize): ReadiumImage? =
        cover()?.scaledToFit(maxSize)
}

/**
 * Returns the publication cover as a [ReadiumImage] at its maximum size.
 */
public suspend fun Publication.cover(): ReadiumImage? =
    findService(CoverService::class)?.cover()?.let { return it }

/**
 * Returns the publication cover as a [ReadiumImage], scaled down to fit the given [maxSize].
 */
public suspend fun Publication.coverFitting(maxSize: ImageSize): ReadiumImage? =
...
internal class ResourceCoverService(
    ...
    override suspend fun cover(): ReadiumImage? =
        decodeCover(maxSize = null)

    override suspend fun coverFitting(maxSize: ImageSize): ReadiumImage? =
        decodeCover(maxSize)

    private suspend fun decodeCover(maxSize: ImageSize?): ReadiumImage? {
        val resource = container[coverUrl]
            ?: return null

        return resource
            .read()
            .flatMap { it.decodeImage(maxSize) }
            .getOrNull()
    }
...
/**
 * A [CoverService] which uses a provided in-memory cover.
 */
public class InMemoryCoverService internal constructor(private val cover: ReadiumImage) : CoverService {

    public companion object {
        public fun createFactory(cover: ReadiumImage?): ServiceFactory = {
            cover?.let { InMemoryCoverService(it) }
        }
    }

    public override suspend fun cover(): ReadiumImage = cover
}
