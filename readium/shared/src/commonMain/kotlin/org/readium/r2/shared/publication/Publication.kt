...
public class Publication(
    ...
    public class ServicesBuilder private constructor(
        ...
        public constructor(
            ...
            mapOf(
                serviceKey(CacheService::class) to cache,
                serviceKey(ContentService::class) to content,
                serviceKey(ContentProtectionService::class) to contentProtection,
                serviceKey(CoverService::class) to cover,
                serviceKey(LocatorService::class) to locator,
                serviceKey(PositionsService::class) to positions,
                serviceKey(SearchService::class) to search
        ...
        public fun build(context: Service.Context): List<Service> {
            val serviceFactories =
                buildMap<String, ServiceFactory> {
                    ...
                    if (!containsKey(serviceKey(LocatorService::class))) {
                        val factory: ServiceFactory = {
                            DefaultLocatorService(it.manifest.readingOrder, it.services)
                        }
                        put(serviceKey(LocatorService::class), factory)
                    }

                    if (!containsKey(serviceKey(CoverService::class))) {
                        val factory = ResourceCoverService.createFactory()
                        put(serviceKey(CoverService::class), factory)
                    }
                }
        ...
        public operator fun <T : Service> get(serviceType: KClass<T>): ServiceFactory? =
            serviceFactories[serviceKey(serviceType)]
        ...
        public operator fun <T : Service> set(serviceType: KClass<T>, factory: ServiceFactory?) {
            val key = serviceKey(serviceType)
            if (factory != null) {
        ...
        public fun <T : Service> remove(serviceType: KClass<T>) {
            serviceFactories.remove(serviceKey(serviceType))
        }
        ...
        private companion object {
            /** Key identifying a service type in the factories map. */
            fun serviceKey(serviceType: KClass<*>): String =
