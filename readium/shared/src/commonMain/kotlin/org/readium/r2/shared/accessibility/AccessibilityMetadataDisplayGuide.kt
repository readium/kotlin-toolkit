...
public class AccessibilityMetadataDisplayGuide(
    ...
    public sealed interface Statement {
        ...
        // Use the platform `localizedString()` extensions to get a localized representation of
    ...
    internal class DynamicStatement(
        internal val compactString: String,
        internal val descriptiveString: String = compactString,
    ) : Statement
}
 ...
 *
 * The [key] matches the resource names of the official W3C localized strings, without the
 * `readium_a11y_` prefix and the `_compact`/`_descriptive` suffix. The actual localization is
 * platform-specific: Android resolves the `R.string` resources, while iOS uses a bundled table of
 * the en-US strings.
 *
