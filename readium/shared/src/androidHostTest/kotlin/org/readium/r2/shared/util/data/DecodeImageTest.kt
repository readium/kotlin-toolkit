...
/**
 * Android side of the `decodeImage` tests; the iOS side lives in `iosTest`.
 *
 * These tests cannot run from `commonTest` because `BitmapFactory` requires Robolectric on the
 * Android host.
 */
...
class DecodeImageTest {
    ...
    // Note: no `decodeInvalidImageFails` test here (unlike iosTest): the Robolectric
    // BitmapFactory shadow decodes any byte array into a placeholder bitmap.
}
