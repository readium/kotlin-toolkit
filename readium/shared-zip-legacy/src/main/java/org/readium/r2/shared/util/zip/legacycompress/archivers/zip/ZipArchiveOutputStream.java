...
import static org.readium.r2.shared.util.zip.legacycompress.archivers.zip.ZipLong.putLong;

import org.readium.r2.shared.util.zip.legacycompress.archivers.ArchiveEntry;
import org.readium.r2.shared.util.zip.legacycompress.archivers.ArchiveOutputStream;
import org.readium.r2.shared.util.zip.legacyjvm.SeekableByteChannel;
import org.readium.r2.shared.util.zip.legacycompress.utils.ByteUtils;
...
public class ZipArchiveOutputStream extends ArchiveOutputStream {
    ...
    private byte[] createCentralFileHeader(final ZipArchiveEntry ze, final ByteBuffer name,
        ...
        if (entryMetaData.offset >= ZipConstants.ZIP64_MAGIC || zip64Mode == Zip64Mode.Always) {
            putLong(ZipConstants.ZIP64_MAGIC, buf, CFH_LFH_OFFSET);
        } else {
            putLong(Math.min(entryMetaData.offset, ZipConstants.ZIP64_MAGIC), buf, CFH_LFH_OFFSET);
        }
    ...
    private void rewriteSizesAndCrc(final boolean actuallyNeedsZip64)
        ...
        if (hasZip64Extra(entry.entry)) {
            ...
            channel.position(entry.localDataStart + 3 * ZipConstants.WORD + 2 * ZipConstants.SHORT
                    + nameLen + 2 * ZipConstants.SHORT);
