...
import org.readium.r2.streamer.parser.defaultCacheServiceFactory
 ...
 * EPUB profile.
 * @param cacheServiceFactory Factory for the cache service attached to the parsed [Publication].
 * Defaults to an in-memory cache on Android (without memory-pressure callbacks, unless you use
 * the Context-taking Android entry point) and to no cache service on iOS.
 */
...
public class ReadiumWebPubParser(
    ...
    private val cacheServiceFactory: ((Publication.Service.Context) -> CacheService)? = defaultCacheServiceFactory(),
) : PublicationParser {
    ...
    private suspend fun parseContainerAsset(
        ...
        val servicesBuilder = Publication.ServicesBuilder().apply {
            cacheServiceFactory = this@ReadiumWebPubParser.cacheServiceFactory
    ...
    private suspend fun parseResourceAsset(
        ...
        if (baseUrl == null) {
            ReadiumLog.w("No self link found in the manifest at ${resource.sourceUrl}")
        } else {
