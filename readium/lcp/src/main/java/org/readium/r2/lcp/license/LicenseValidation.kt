...
internal class LicenseValidation(
    ...
    val isProduction: Boolean = {
        ...
        try {
            LcpClient.findOneValidPassphrase(prodLicense.json, listOf(passphrase)) == passphrase
        } catch (e: Exception) {
            false
        }
    }()
...
//        Timber.d("observers $observers")
    ...
    private suspend fun validateIntegrity(license: LicenseDocument, passphrase: String) {
        ...
        val context = LcpClient.createContext(license.json, passphrase, crl.retrieve())
