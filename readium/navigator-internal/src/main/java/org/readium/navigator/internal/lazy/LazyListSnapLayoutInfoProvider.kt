...
fun rememberSnapFlingBehavior(lazyListState: LazyListState): FlingBehavior {
    val snappingLayout = remember(lazyListState) {
        org.readium.navigator.internal.lazy.SnapLayoutInfoProvider(
            lazyListState
        )
    }
    return androidx.compose.foundation.gestures.snapping.rememberSnapFlingBehavior(snappingLayout)
}
