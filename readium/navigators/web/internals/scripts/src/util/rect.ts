 ...
 */
export function dezoomDomRect(rect: DOMRect, zoomLevel: number): DOMRect {
  return new DOMRect(
    rect.x / zoomLevel,
    rect.y / zoomLevel,
 ...
 * See https://issues.chromium.org/issues/40391865#comment9
 */
export function dezoomRect(rect: Rect, zoomLevel: number): Rect {
  return {
...
export function getClientRectsNoOverlap(
  ...
  const tolerance = 1
  const originalRects: Array<Rect> = []
...
function removeContainedRects(rects: Array<Rect>, tolerance: number) {
...
function replaceOverlapingRects(rects: Array<Rect>) {
  for (let i = 0; i < rects.length; i++) {
    for (let j = i + 1; j < rects.length; j++) {
      const rect1 = rects[i]
      const rect2 = rects[j]
